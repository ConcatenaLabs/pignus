...
class BtcLoan:
    ...
    upgrade_fee: int = 3000     # satoshis the pre-vault carries for the move
    ...
    def prevault_tree(self):
        ...
        bx = bytes.fromhex(self.borrower_x)
        return B.TapTree(B.NUMS, [
            ("upgrade", B.hashlocked_single(bytes.fromhex(self.h_w), bx)),
...
def upgrade_tx(loan, prevault_txid, vout):
    ...
    the vault's outpoint -- and therefore the reclaim the lender adaptor-signs
...
def complete_upgrade(loan, prevault_txid, vout, presig, secret_w):
    """Finish the move once `w` is public on Sequentia. Anyone can broadcast
    it; the lender is simply the one who wants to."""
    ...
    tree = loan.prevault_tree()
    if not A.verify(bytes.fromhex(loan.borrower_x),
                    _prevault_sighash(loan, tx, "upgrade"), presig):
        raise ValueError("the borrower's advance signature does not verify")
    tx.vin[0].witness = [presig, secret_w, tree.leaves["upgrade"],
...
def _spend_hashlock(node, tap, leaves, txid, vout, leaf, *, value, asset,
    ...
    if fee_asset is None or fee is None:
        chosen, atoms = seq_fee_choice(node, prefer=(asset,), flow="repay4")
        fee_asset = fee_asset or chosen
        fee = int(fee if fee is not None else atoms)
...
def fund_bitcoin(btc_node, loan, feerate=5, broadcast=True, prevault=None):
    ...
    broadcasting when the caller wants the txid first: the lender's adaptor
...
def collateral_committed(btc_node, loan, funding_txid, funding_vout,
    ...
    if out is None:
        return False, "that outpoint is unspent nowhere: it is spent or unknown"
    ...
    if conf < int(min_conf):
        return False, (f"the collateral has {conf} confirmation(s), below the "
                       f"{min_conf} required")
